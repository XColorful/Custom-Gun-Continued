...
import dev.xcolorful.customgun.client.animation.ObjectAnimationRunner;
import dev.xcolorful.customgun.client.animation.controller.AnimController;
import dev.xcolorful.customgun.client.animation.statemachine.DiscreteTrackArray;
...
public class AnimStateContext {
     ...
     * @return 上下文绑定的状态机
     */
    public @Nullable AnimStateMachine<?> getStateMachine() {
        return stateMachine;
    }
     ...
     * @return 轨道在控制器中的指针
     * @throws TrackArrayMismatchException 当状态机对应的 track array 不是当前 context 指定的实例，抛出此异常。
     * @see AnimStateContext#assignNewTrack(int)
     */
    public int findIdleTrack(int index, boolean interruptHolding) {
        ...
        AnimController controller = stateMachine.getAnimationController();
    ...
    private static final AnimationPlayType[] ANIMATION_TYPES = AnimationPlayType.values();
    /**
    ...
    public boolean hasAnimationPrototype(String name) {
        var stateMachine = checkStateMachine();
        AnimController animationController = stateMachine.getAnimationController();
    ...
    void setStateMachine(@Nullable AnimStateMachine<?> stateMachine) {
    ...
    private @NotNull AnimStateMachine<?> checkStateMachine() {
        if (this.stateMachine == null) {
