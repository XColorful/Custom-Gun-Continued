...
public record C2SMessageShooterRefit_install(int attachmentSlotIndex,
                                             int gunSlotIndex,
                                             AttachmentCategory attachmentCategory)
        implements IMessage<C2SMessageShooterRefit_install> {

    @Override
    public void encode(C2SMessageShooterRefit_install message, FriendlyByteBuf buffer) {
    ...
    public static C2SMessageShooterRefit_install decode(FriendlyByteBuf buffer) {
        return new C2SMessageShooterRefit_install(buffer.readInt(), buffer.readInt(), buffer.readEnum(AttachmentCategory.class));
    }

    @Override
    public void handle(C2SMessageShooterRefit_install message, Consumer<Runnable> handler, NetworkContext context) {
        if (CustomGun.getSideExecutor().getLogicalSide().isServer()) {
            handler.accept(() -> {
                ...
                SendUtils.sendMessageToPlayer(player, new S2CMessageShooterRefit_refresh());
            });
        }
    }
}