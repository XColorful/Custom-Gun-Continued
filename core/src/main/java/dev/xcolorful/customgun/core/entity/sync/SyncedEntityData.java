...
public class SyncedEntityData {
    ...
    public synchronized <E extends Entity, T> void registerDataKey(SyncedDataKey<E, T> dataKey) {
        var keyId = dataKey.id();
        SyncedClassKey<E> classKey = dataKey.classKey();
        if (CommonSetup.get().isLoadComplete()) {
        ...
        CustomGun.LOGGER.info(SYNCED_ENTITY_DATA_MARKER, "Registered synced data key {} for {}", dataKey.id(), classKey.id());
    }
    ...
    public <E extends Entity, T> void set(E entity, SyncedDataKey<?, ?> key, T value) {
        if (!this.registeredDataKeys.contains(key)) {
            ...
            CustomGun.LOGGER.info(SYNCED_ENTITY_DATA_MARKER, "Registered keys before throwing exception: {}", keys);
            throw new IllegalArgumentException(String.format("The synced data key %s for %s is not registered!", key.id(), key.classKey().id()));
        }
        SyncDataHolder holder = this.getSyncDataHolder(entity);
    ...
    @Nullable
    public SyncDataHolder getSyncDataHolder(Entity entity) {
        return CustomGun.getCapabilityProvider().getSyncDataHolder(entity, null);
    }
    ...
    public boolean updateMappings(ServerMessageSyncedEntityDataMapping message) {
        ...
        if (!missingKeys.isEmpty()) {
            ...
            CustomGun.LOGGER.info(SYNCED_ENTITY_DATA_MARKER, "Received unknown synced keys: {}", keys);
        }
