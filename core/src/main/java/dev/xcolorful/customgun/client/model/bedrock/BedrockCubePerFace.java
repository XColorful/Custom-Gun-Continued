...
import dev.xcolorful.customgun.core.util.ColorUtils;
...
public class BedrockCubePerFace implements IBedrockCube {
    ...
    public BedrockCubePerFace(float x, float y, float z,
    ...
    private BedrockPolygon getTexturedQuad(BedrockVertex[] positionsIn, float texWidth, float texHeight, Direction direction, _Uv faces) {
        _FaceUv face = faces.getFaceUv(direction);
    ...
    public void compile(PoseStack.Pose pose,
        ...
        for (int i = 0; i < this.polygons.length; i++) {
            BedrockPolygon polygon = this.polygons[i];
            ...
            for (int j = 0; j < polygon.vertices.length; j++) {
                BedrockVertex vertex = polygon.vertices[j];
                ...
                consumer.addVertex(vector4f.x(), vector4f.y(), vector4f.z(),
                        ColorUtils.ARGB32.of(alpha, red, green, blue),
