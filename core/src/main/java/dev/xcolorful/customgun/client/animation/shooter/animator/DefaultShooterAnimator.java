...
public class DefaultShooterAnimator implements IShooterAnimator {
    ...
    public void animateShooter(ModelPart head, ModelPart body, ModelPart leftArm, ModelPart rightArm,
        ...
        // 1. 瞄准动画
        {
            ...
            if (mainArmAlwaysAim) {
                this._animateShooterAimingMainArm(head, mainArm);
     ...
     *         <li>原版在 {@code this.crouching} 分支里对双臂 {@code xRot += 0.4F}，跟随则蹲下时枪口下压 0.4 rad（约 23°）</li>
     ...
     * </ul>
     */
    public static void _animateShooterAimingMainArm(ModelPart head, ModelPart arm) {
        arm.yRot = head.yRot;
     ...
     * <ul>
     *     <li>起点取原版 {@link HumanoidModel#poseRightArm} / {@link HumanoidModel#poseLeftArm} 的 {@link HumanoidModel.ArmPose#BOW_AND_ARROW} 分支之持械手</li>
     *     <li>起点值：{@code yRot = ∓0.1 + head.yRot}，{@code xRot = -PI/2 + head.xRot}</li>
     *     <li>原版由 {@link PlayerRenderer#getArmPose} 在拉弓时选中该分支</li>
     *     <li>终点为 {@link #_animateShooterAimingMainArm}</li>
     * </ul>
     */
     ...
     * <ul>
     *     <li>起点取原版 {@link AnimationUtils#animateCrossbowHold} 的持械手</li>
     *     <li>起点值：{@code yRot = ∓0.3 + head.yRot}，{@code xRot = -PI/2 + head.xRot + 0.1}</li>
     *     <li>{@link HumanoidModel.ArmPose#CROSSBOW_HOLD}，原版由 {@link PlayerRenderer#getArmPose} 在弩已蓄满且玩家未挥手时选中</li>
     *     <li>终点为 {@link #_animateShooterAimingMainArm}</li>
     *     <li>与弓的持械手相比：多一项 {@code xRot} 的 {@code +0.1} 俯仰偏移；{@code ∓0.3} 偏航远大于弓的 {@code ∓0.1}</li>
     ...
     * 弓的副手姿态
     * <ul>
     *     <li>起止相同，即不随瞄准进度变化</li>
     *     <li>原版 {@code BOW_AND_ARROW} 分支之副手：{@code yRot = ±0.1 + head.yRot ± 0.4}，{@code xRot = -PI/2 + head.xRot}</li>
    ...
    public static void _animateShooterAimingBowOffArm(ModelPart head, ModelPart arm, boolean mainArmIsRight,
        ...
        _applyArmAimPose(arm, poseWhenIdle, animationProgress, yRot, xRot, yRot, xRot);
    }
     ...
     * <ul>
     *     <li>起止相同，即不随瞄准进度变化</li>
     *     <li>原版 {@link AnimationUtils#animateCrossbowHold} 之副手：{@code yRot = ±0.6 + head.yRot}，{@code xRot = -1.5 + head.xRot}</li>
     *     <li>副手不挂枪械，未做偏离，也不收敛到视线朝向</li>
     * </ul>
     */
