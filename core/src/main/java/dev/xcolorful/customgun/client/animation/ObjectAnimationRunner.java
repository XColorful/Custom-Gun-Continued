...
public class ObjectAnimationRunner {
    ...
    private final @NotNull ObjectAnimation animation;
    ...
    private @Nullable ObjectAnimationRunner transitionTo;
            ...
            for (Map.Entry<String, List<ObjectAnimationChannel>> entry : animation.getChannels().entrySet()) {
                ...
                if (toChannels != null) {
                    ...
                    for (ObjectAnimationChannel channel : entry.getValue()) {
                        ...
                        if (channel.type == AnimationChannelType.ROTATION && value.length == 3) {
                            value = MathUtil.Quaternion.fromEulerAngles(value[0], value[1], value[2]);
                        }
            ...
            for (int i = 0; i < transitionFromChannels.size(); i++) {
                ...
                switch (fromChannel.type) {
                    ...
                    case ROTATION -> {
                        result = new float[4];
                        if (to.length == 3) {
                            to = MathUtil.Quaternion.fromEulerAngles(to[0], to[1], to[2]);
                        }
    ...
    public void update(boolean blend) {
        ...
        if (isTransitioning) {
            ...
            if (soundChannel != null && mc.player != null) {
                soundChannel.playSound(fromTimeNs / 1e9, progressNs / 1e9, mc.player, 16, 1 ,1);
            }
        }
    }
    ...
    private void updateTransition(float progress, boolean blend) {
        ...
        if (animation.playType == AnimationPlayType.PLAY_ONCE_STOP) return;
        ...
        for (int i = 0; i < recoverChannels.size(); i++) {
            ...
            switch (channel.type) {
                case TRANSLATION -> {
                    ...
                    for (IAnimationListener listener : channel.getListeners()) {
                        lerp(from, to, progress, result);
                        listener.update(result, blend);
                    }
                }
