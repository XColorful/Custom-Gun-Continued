...
public class AnimController {
    ...
    private final IAnimationListenerSupplier listenerSupplier;
    private final ArrayList<Queue<AnimPlan>> animationQueue = new ArrayList<>();
    ...
    public AnimController(List<ObjectAnimation> animationPrototypes, IAnimationListenerSupplier model) {
    ...
    public void queueAnimation(int track, Queue<AnimPlan> queue) {
        // 确保数组长度正确
    ...
    public void runAnimation(int track, String animationName, AnimationPlayType playType, float transitionTimeS) {
    ...
    private void updateByTrack(int track, boolean isSoundOnly) {
        ...
        if ((runner.isHolding() || runner.isStopped()) && !runner.isTransitioning()) {
            if (track < animationQueue.size()) {
                Queue<AnimPlan> queue = animationQueue.get(track);
                if (queue != null) {
                    AnimPlan plan = null;
                    while (plan == null && !queue.isEmpty()) {
                        plan = queue.poll();
