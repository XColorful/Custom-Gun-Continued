...
|子体系|数据存储|脚本内容|消费方式|
|---|---|---|---|
|**枪械行为脚本**|`GunData.script` 引用的`.lua`文件|名为`start_bolt`/`shoot`/`tick_reload` 等的 Lua 函数，接收`GunScriptApi`|`ModernKineticGunItem` 各处硬编码调用 → CGC 计划分散到各`GunSubManager`|
...
|**管理器层**|`gun.script`|脚本行为扩展点（当前为空壳）|`GunScriptManager`, `IGunScriptManager`, `IGunScriptRuntime`|
...
- `IGunScriptRuntime`是空接口，等待脚本行为实现注入。CGC 的设计意图是让枪械逻辑不再像 TaCZ `ModernKineticGunItem`那样大而全，而是将行为分散到`IGunAttackRuntime`/`IGunActionRuntime`等 sub-manager
- TaCZ `ModernKineticGunScriptAPI`中大而全的方法（`getReloadTime`/`adjustShootInterval`等）在 CGC 中由`ILivingShooter`及其组合接口提供，`GunScriptApi`仅做上下文聚合

## 文档导航

|文档|内容|
|---|---|
|[GunScriptManager](./gun-script-manager.md)|`IGunScriptManager`接口定义、`GunScriptManager`实现（脚本行为扩展点）|
...
|---|---|---|---|
|**Gun behavior scripts**|`.lua` files referenced by `GunData.script`|Lua functions named `start_bolt`/`shoot`/`tick_reload` etc., receiving `GunScriptApi`|Hardcoded calls scattered throughout `ModernKineticGunItem` → CGC plans to distribute to `GunSubManager`s|
...
|**Manager layer**|`gun.script`|Script behavior extension point (currently a stub)|`GunScriptManager`, `IGunScriptManager`, `IGunScriptRuntime`|
...
- `IGunScriptRuntime` is an empty interface, awaiting script behavior implementation injection. The design intent is to avoid the monolithic `ModernKineticGunItem` approach, distributing behavior across `IGunAttackRuntime`/`IGunActionRuntime` and other sub-managers
- The many methods in TaCZ's `ModernKineticGunScriptAPI` (`getReloadTime`/`adjustShootInterval` etc.) are provided by `ILivingShooter` and its composed interfaces in CGC; `GunScriptApi` is purely a context aggregator

## Document Navigation

|Document|Content|
|---|---|
|[GunScriptManager](./gun-script-manager.md)|`IGunScriptManager` interface definition, `GunScriptManager` implementation (script behavior extension point)|
